...
serviciosDentalesData.forEach(servicio => {
    ...
    servicioLi.textContent = servicio.nombre;
...
const solicitarServicios = () => {
    ...
    servicioSeleccionadoTextoHeader.textContent = "Ingresa el servicio que se requiere sin acentos o escribe 'listo' para finalizar";
    ...
    servicioSeleccionadoButton.addEventListener('click', () => {
        ...
        if (servicioEncontrado) {
            ...
            servicioAgregadoDiv.textContent = `Servicio Agregado: ${servicioSeleccionado} - Costo: $${costoServicio}`;
            serviciosAgregadosDiv.appendChild(servicioAgregadoDiv);
        } else {
            const servicioNoDisponibleDiv = document.createElement('div');
            servicioNoDisponibleDiv.textContent = "Lo siento mucho, el servicio encontrado no está disponible";
            serviciosAgregadosDiv.appendChild(servicioNoDisponibleDiv);
        }
    });
};
        ...
        if (metodoPago === "efectivo" || metodoPago === "tarjeta") {
            switch (metodoPago) {
                case "tarjeta":
                    ...
                    const costoTotalConTarjetaDiv = document.createElement('div');
                    costoTotalConTarjetaDiv.textContent = `El costo total de los servicios seleccionados más la comisión de pago con tarjeta es: $${costoTotalConTarjeta}`;
                    container.appendChild(costoTotalConTarjetaDiv);
                    break;
                case "efectivo":
                    const costoTotalEfectivoDiv = document.createElement('div');
                    costoTotalEfectivoDiv.textContent = `El costo total de los servicios seleccionados es: $${costoTotal}`;
                    container.appendChild(costoTotalEfectivoDiv);
                    break;
            }
            finalizar();
        } else {
            const metodoPagoInvalidoDiv = document.createElement('div');
            metodoPagoInvalidoDiv.textContent = "Favor de ingresar si se pagará con efectivo o tarjeta.";
            container.appendChild(metodoPagoInvalidoDiv);
        }
    });
};
